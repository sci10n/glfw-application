...
Texture::Texture(const unsigned width, const unsigned height, GLuint type, GLuint format): _width{width}, _height{height} {
    _id = OpenGLUtils::createTexture(width,height,type,format);
    _valid = true;
}
...
void Texture::begin(int textureUnit) const{
